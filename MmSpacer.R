...
### arrange 2,top/bottom spacers with RNA---NNN
WT<-read.delim("MMB1_WT_sp_detail.info")
RTD<-read.delim("MMB1_RTdelta_sp_detail.info")
...
## Fig1A, top/bottom spacers with RNA---NNN
pdf("FigS1A.pdf",width=8,height=4)
par(mfrow=c(1,2),cex=0.7)
#from the start of the soft-clipped seq (+1 to +5 from the end of the sapcer)
plotspcar_stack5<-function(data_df,title,y_max){
  tmp<-data.frame("Nucl"=c("A","G","C","T"))
  for (i in 1:5){
...
plotspcar_stack3<-function(data_df,title,y_max){
  tmp<-data.frame("Nucl"=c("A","G","C","T"))
  for (i in 1:5){
...
df1<-WT[WT$Sp_type!="U",]
df1<-df1[df1$SSeq3!="",]
plotspcar_stack3(df1,"WT",6000)

df1<-RTD[RTD$Sp_type!="U",]
df1<-df1[df1$SSeq3!="",]
plotspcar_stack3(df1,"RTD",1000)
